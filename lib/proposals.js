...
// Lighter-weight than buildProposals -- only fetches companies + matches
// partners, skipping end-client/churned matching and branch detection (not
// needed for a partners export). Manel asked (14/09/2026) for an Excel of
// partners: EVERY company already tagged tipo_de_empresa='Partner' in
// HubSpot, not just the ones buildProposals would surface (that list only
// includes companies with something still PROPOSED -- a partner that's
// already fully filled in never appears there), plus the partners that exist
// in nsign's own device-history but have no HubSpot company at all yet.
async function buildPartnersExport(onProgress) {
  const report = onProgress || (() => {});
  report({ stage: 'Descargando companies de HubSpot…', done: 0, total: null });
  const companies = await getAllCompanies((fetched) => report({ stage: `Descargando companies de HubSpot… (${fetched} traídas)`, done: fetched, total: null }));
  ...
  const histByCompanyId = new Map();
  for (const [name, match] of Object.entries(partnerMatches)) {
    if (match) histByCompanyId.set(match.company.id, { name, ...partnerHistory[name] });
  }

  const partners = companies
    .filter((c) => c.properties.tipo_de_empresa === 'Partner')
    .map((c) => {
      const hist = histByCompanyId.get(c.id) || null;
      return {
        id: c.id,
        name: c.properties.name,
        company_score: c.properties.company_score || null,
        relacion: c.properties.relacion || null,
        estado_del_partner: c.properties.estado_del_partner || null,
        country: c.properties.country || null,
        numero_de_dispositivos: c.properties.numero_de_dispositivos || null,
        website: c.properties.website || null,
        activeDevicesInHistory: hist ? hist.active_devices : null,
        managedClientCountInHistory: hist ? hist.managed_client_count : null,
        url: `https://app.hubspot.com/contacts/${process.env.HUBSPOT_PORTAL_ID || ''}/record/0-2/${c.id}`,
      };
    })
    .sort((a, b) => (b.activeDevicesInHistory || 0) - (a.activeDevicesInHistory || 0));
  ...
  report({ stage: 'Listo', done: 1, total: 1 });
  return { partners, unmatchedPartners, totalCompaniesScanned: companies.length };
}

// Real-data approximation of "revenue split by vertical" for an external
// partner form (Manel, 17/09/2026) -- active_devices as the revenue proxy
// (more screens deployed ≈ more platform revenue from that client), since no
// real per-client revenue figure is available here. For each active client
// in nsign's own device history, resolve a vertical from whichever is more
// authoritative: the company's OWN sector__vertical_ property already set in
// HubSpot (preferred -- it's the richer, already-curated taxonomy), falling
// back to the Excel-enrichment-derived value (same mapping/low-trust gate
// buildProposals itself uses) when HubSpot has nothing. Clients that match
// neither are reported separately as unclassified rather than silently
// dropped, since that fraction matters for how much to trust the split.
async function buildVerticalDeviceApprox(onProgress) {
  const report = onProgress || (() => {});
  report({ stage: 'Descargando companies de HubSpot…', done: 0, total: null });
  const companies = await getAllCompanies((fetched) => report({ stage: `Descargando companies de HubSpot… (${fetched} traídas)`, done: fetched, total: null }));
  const companiesById = new Map(companies.map((c) => [String(c.id), c]));
  ...
  const clientNames = Object.keys(clientHistory);
  report({ stage: `Cruzando ${clientNames.length} clientes del historial de players…`, done: 0, total: clientNames.length });
  const clientMatches = matchClientsToCompanies(clientNames, companies, (done, total) => report({ stage: 'Cruzando clientes…', done, total }));

  const byVertical = new Map(); // vertical string -> { devices, clientCount, clients: [] }
  let unclassifiedDevices = 0;
  let unclassifiedClientCount = 0;
  let unmatchedDevices = 0; // no HubSpot company at all
  let unmatchedClientCount = 0;

  for (const name of clientNames) {
    const hist = clientHistory[name];
    const devices = hist.active_devices || 0;
    if (devices <= 0) continue; // only active clients -- churned ones live in a separate file
    const match = clientMatches[name];
    if (!match) {
      unmatchedDevices += devices;
      unmatchedClientCount += 1;
      continue;
    }
    const company = match.company;
    let vertical = company.properties.sector__vertical_ || null;
    let source = 'hubspot_sector_vertical';
    if (!vertical) {
      const excelRow = excelEnrichment[String(company.id)];
      if (excelRow && excelRow.industry_sector_excel) {
        const lowTrust = LOW_TRUST_SECTORS.has(excelRow.industry_sector_excel) && excelRow.research_confidence !== 'high';
        const mapped = EXCEL_SECTOR_TO_SECTOR_VERTICAL[excelRow.industry_sector_excel];
        if (mapped && !lowTrust) {
          vertical = mapped;
    ...
    if (!vertical) {
      unclassifiedDevices += devices;
      unclassifiedClientCount += 1;
      continue;
    }
    if (!byVertical.has(vertical)) byVertical.set(vertical, { devices: 0, clientCount: 0, clients: [], source });
    const agg = byVertical.get(vertical);
    agg.devices += devices;
    agg.clientCount += 1;
    agg.clients.push({ name, devices, companyId: company.id });
  }

  const verticals = [...byVertical.entries()]
    .map(([vertical, agg]) => ({ vertical, devices: agg.devices, clientCount: agg.clientCount, source: agg.source }))
    .sort((a, b) => b.devices - a.devices);

  const totalActiveDevices = verticals.reduce((s, v) => s + v.devices, 0) + unclassifiedDevices + unmatchedDevices;

  report({ stage: 'Listo', done: 1, total: 1 });
  return {
    verticals,
    unclassifiedDevices, unclassifiedClientCount,
    unmatchedDevices, unmatchedClientCount,
    totalActiveDevices,
  };
}

module.exports = { buildProposals, buildPartnersExport, buildVerticalDeviceApprox };
