...
// Newsletter/comms-access proposal -- NOT a HubSpot field until Manel creates
// the custom property; we only ever *propose* a value here.
function proposeCommsAccess({ isDirectChannel, country, hasExternalPartner }) {
  if (isDirectChannel === false && hasExternalPartner) {
    // Served via an external partner -- outside ES/PT that is the required
    // model; inside ES/PT it's one valid model among others.
    return 'Solo via Partner';
  }
  if (isDirectChannel && !SPAIN_PORTUGAL.has(country)) {
    // Direct relationship outside ES/PT breaks the partner-only policy.
    return 'Directo (¡revisar! fuera de ES/PT)';
  }
  if (isDirectChannel) return 'Directo';
...
async function buildProposals() {
  ...
  for (const clientName of clientNames) {
    ...
    p.proposed.acceso_comercial_newsletter = commsAccess; // NOT written unless the property exists in HubSpot
    if (commsAccess.startsWith('Directo (¡revisar!')) {
