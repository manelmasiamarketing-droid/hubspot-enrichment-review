...
async function buildInventory(tokenB, monthsBack, onProgress, activityMonths) {
  ...
  // Checked across BOTH duplicates and net-new in one batched pass -- not
  // just net-new -- specifically so the duplicates count doubles as a sanity
  // check of the mechanism itself. Manel's numbers (09/09/2026) showed 0 of
  // 4653 net-new companies with a real email at 12 AND 24 months, which is
  // suspicious enough to need proof the check can find ANY company-level
  // email association in this portal at all: if the known-real duplicates
  // (companies that already exist in both accounts) ALSO show 0, that means
  // this portal doesn't associate emails directly to Companies (likely only
  // to Contacts), and the net-new "0%" isn't a real finding -- just this
  // check's blind spot.
  const allForActivityCheck = [...duplicates, ...netNew];
  report({ stage: `Cruzando ${allForActivityCheck.length} companies (duplicados + net-new) contra actividad real de email (últimos ${emailActivityMonths} meses)…`, done: 0, total: null });
  const emailActivity = await evaluateRecentEmailActivity(
    hsFetchB,
    allForActivityCheck.map((n) => n.idInB),
    emailActivityMonths,
    (stage) => report({ stage, done: 0, total: null }),
  );
  let netNewWithRealActivity = null;
  let duplicatesWithRealActivity = null;
  ...
  if (emailActivity.error) {
    ...
    duplicates.forEach((d) => { d.recentEmailActivity = 'error'; });
  } else {
    netNewWithRealActivity = 0;
    duplicatesWithRealActivity = 0;
    ...
    for (const d of duplicates) {
      const active = emailActivity.activeCompanyIds.has(d.idInB);
      d.recentEmailActivity = active;
      if (active) duplicatesWithRealActivity += 1;
    }
  }
