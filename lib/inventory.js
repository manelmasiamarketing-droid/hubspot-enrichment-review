...
function filterGroupsFor(monthsBack) {
  const cutoff = cutoffTimestamp(monthsBack);
  if (cutoff === null) return [];
  return [{ filters: [{ propertyName: 'hs_lastmodifieddate', operator: 'GTE', value: cutoff }] }];
}
...
async function buildInventory(tokenB, monthsBack, onProgress) {
  ...
  const filterGroups = filterGroupsFor(monthsBack);
  const cutoff = cutoffTimestamp(monthsBack);
  ...
  for (const objectType of COUNT_ONLY_OBJECT_TYPES) {
    ...
    try {
      objectCounts[objectType] = await countTotal(hsFetchB, objectType, filterGroups);
    } catch (e) {
  ...
  return {
    filter: {
      ...
      description: sinceIso
        ? `Solo objetos de la cuenta B con hs_lastmodifieddate en los últimos ${monthsBack} meses`
