...
// Manel asked (09/09/2026) to scope the whole inventory to objects updated in
// the last 6 months only, not the entire account -- `hs_lastmodifieddate` is
// the standard "last touched" property HubSpot maintains on every CRM object
// type (companies, contacts, deals, and every engagement type), so the same
// filter works unchanged across all of them.
function sixMonthsAgoTimestamp() {
  const d = new Date();
  d.setMonth(d.getMonth() - 6);
  return d.getTime();
}

function recentFilterGroups() {
  return [{
    filters: [{ propertyName: 'hs_lastmodifieddate', operator: 'GTE', value: sixMonthsAgoTimestamp() }],
  }];
}

async function countTotal(hsFetch, objectType) {
  const page = await hsFetch(`/crm/v3/objects/${objectType}/search`, {
    method: 'POST',
    body: JSON.stringify({ limit: 1, filterGroups: recentFilterGroups() }),
  });
  return page.total;
}

async function fetchAllCompaniesB(hsFetch, onProgress) {
  const all = [];
  let after;
  do {
    const body = {
      limit: 100,
      properties: ['name'],
      filterGroups: recentFilterGroups(),
      ...(after ? { after } : {}),
    };
...
async function buildInventory(tokenB, onProgress) {
  const report = onProgress || (() => {});
  const hsFetchB = hsFetchWithToken(tokenB);
  const sinceIso = new Date(sixMonthsAgoTimestamp()).toISOString().slice(0, 10);

  const objectCounts = {};
  for (const objectType of COUNT_ONLY_OBJECT_TYPES) {
    report({ stage: `Cuenta B: contando ${objectType} actualizados desde ${sinceIso}…` });
    try {
      objectCounts[objectType] = await countTotal(hsFetchB, objectType);
    } catch (e) {
      objectCounts[objectType] = { error: String(e.message || e) };
    }
  }

  report({ stage: `Cuenta B: descargando companies actualizadas desde ${sinceIso}…`, done: 0, total: null });
  let companiesB = [];
  try {
    companiesB = await fetchAllCompaniesB(hsFetchB, (n) => report({ stage: `Cuenta B: descargando companies… (${n} traídas)`, done: n, total: null }));
  } catch (e) {
    return {
  ...
  return {
    filter: {
      description: 'Solo objetos de la cuenta B con hs_lastmodifieddate en los últimos 6 meses',
      sinceDate: sinceIso,
    },
