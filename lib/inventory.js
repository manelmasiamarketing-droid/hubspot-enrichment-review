...
// within this fixed window, independent of whichever tab/monthsBack is
// selected, as a much more honest "worth migrating" signal.
const EMAIL_ACTIVITY_MONTHS = 12;
...
async function evaluateRecentEmailActivity(hsFetch, companyIds, report) {
  if (companyIds.length === 0) return { activeCompanyIds: new Set() };
  const cutoff = cutoffTimestamp(EMAIL_ACTIVITY_MONTHS);
...
async function buildInventory(tokenB, monthsBack, onProgress) {
  ...
  report({ stage: `Cruzando ${netNew.length} companies net-new contra actividad real de email (últimos ${EMAIL_ACTIVITY_MONTHS} meses)…`, done: 0, total: null });
  ...
  return {
    ...
    companiesComparison: {
      ...
      emailActivityMonths: EMAIL_ACTIVITY_MONTHS,
      emailActivityError,
      duplicates,
      netNew,
    },
