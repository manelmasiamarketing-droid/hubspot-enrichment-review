...
// Read-only lookup by id (any object type) with chosen properties -- used to
// inspect the real, current state of a specific record in account A before
// deciding on a fix (e.g. the Mediahome/Mediamarkt cleanup, 12/09/2026).
async function getById(objectType, id, properties) {
  try {
    const params = new URLSearchParams();
    (properties || ['name']).forEach((p) => params.append('properties', p));
    return await hsFetch(`/crm/v3/objects/${objectType}/${id}?${params.toString()}`);
  } catch (e) {
...
// Like findByProperty but returns ALL matches (up to `limit`), not just the
// first -- used to see every duplicate of a value (e.g. two "MCR" companies
// both named the same, 12/09/2026) instead of only ever seeing one.
async function searchByProperty(objectType, propertyName, value, properties, limit) {
...
// Batch associations read in account A -- used to check, BEFORE creating
// anything, whether a contact matched by email already belongs to a
// DIFFERENT company than the one about to be created (Manel's point,
// 12/09/2026, re: "Mediahome" turning out to be @mediamarkt.es contacts --
// creating a new company would split a real account in two instead of
// flagging the likely duplicate).
async function batchReadContactCompanies(contactIds) {
  const fromToIds = new Map();
  for (let i = 0; i < contactIds.length; i += 1000) {
    const chunk = contactIds.slice(i, i + 1000);
    const page = await hsFetch('/crm/v4/associations/contacts/companies/batch/read', {
      method: 'POST',
      body: JSON.stringify({ inputs: chunk.map((id) => ({ id })) }),
    });
    for (const r of page.results || []) {
      const toIds = (r.to || []).map((t) => String(t.toObjectId));
      if (toIds.length) fromToIds.set(String(r.from.id), toIds);
    }
  }
  return fromToIds;
}

async function batchReadCompanyNames(companyIds) {
  const names = new Map();
  for (let i = 0; i < companyIds.length; i += 100) {
    ...
    for (const r of page.results || []) names.set(r.id, r.properties && r.properties.name);
  }
  return names;
}
...
module.exports = {
  ...
  createContact,
  associateDefault,
  batchReadContactCompanies,
  batchReadCompanyNames,
  getById,
  searchByProperty,
};
